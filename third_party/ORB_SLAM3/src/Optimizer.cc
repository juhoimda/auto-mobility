...
void Optimizer::FullInertialBA(Map *pMap, int its, const bool bFixLocal, const long unsigned int nLoopId, bool *pbStopFlag, bool bInit, float priorG, float priorA, Eigen::VectorXd *vSingVal, bool *bHess)
{
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        if (!Rcw.hasNaN() && !tcw.hasNaN())
        {
            Sophus::SE3f Tcw(IMU::NormalizeRotation(Rcw), tcw);
            if(nLoopId==0)
            {
                pKFi->SetPose(Tcw);
            }
        ...
        if(pKFi->bImu)
        {
            ...
            if (vb.hasNaN())
            {
                cerr << "FullInertialBA: bias estimate is NaN, skipping bias update" << endl;
                continue;
            }
...
void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &scale, Eigen::Vector3d &bg, Eigen::Vector3d &ba, bool bMono, Eigen::MatrixXd  &covInertial, bool bFixedVel, bool bGauss, float priorG, float priorA)
{
    ...
    if (vb.hasNaN())
    {
        cerr << "InertialOptimization: bias estimate is NaN, aborting" << endl;
        return;
    }
    bg << VG->estimate();
    ba << VA->estimate();
...
{
    if(!pFrame->mpImuPreintegrated)
    {
        cerr << "PoseInertialOptimizationLastKeyFrame: mpImuPreintegrated is null" << endl;
        return 0;
    }
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolverX::LinearSolverType * linearSolver;
    ...
    Eigen::Matrix3f Rwb_est = VP->estimate().Rwb.cast<float>();
    Eigen::Vector3f twb_est = VP->estimate().twb.cast<float>();
    Eigen::Vector3f Vwb_est = VV->estimate().cast<float>();
    if (!Rwb_est.hasNaN() && !twb_est.hasNaN() && !Vwb_est.hasNaN())
    {
        pFrame->SetImuPoseVelocity(Rwb_est, twb_est, Vwb_est);
    }
    Vector6d b;
    b << VG->estimate(), VA->estimate();
    if (!b.hasNaN())
...
int Optimizer::PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit)
{
    ...
    {
        cerr << "PoseInertialOptimizationLastFrame: mpImuPreintegratedFrame is null" << endl;
        return 0;
    }
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolverX::LinearSolverType * linearSolver;
