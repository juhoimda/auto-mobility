...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
{
    ...
    if (!mpCurrentKeyFrame->GetMap()->isImuInitialized())
    {
        ...
        for(vector<KeyFrame*>::iterator itKF = vpKF.begin(); itKF!=vpKF.end(); itKF++)
        {
            ...
            if (!(*itKF)->mPrevKF)
                continue;
            if ((*itKF)->mpImuPreintegrated->dT <= 0.0f)
                continue;
    ...
    if (mbg.hasNaN() || mba.hasNaN() || mRwg.hasNaN() || std::isnan(mScale))
    {
        cout << "IMU init produced NaN, aborting initialization" << endl;
        bInitializing=false;
        return;
    }
...
void LocalMapping::ScaleRefinement()
{
    ...
    if (mRwg.hasNaN() || std::isnan(mScale))
    {
        cout << "IMU init (Rwg) produced NaN, aborting initialization" << endl;
        bInitializing=false;
        return;
    }
