...
{
    if (Rwb.hasNaN() || twb.hasNaN() || Vwb.hasNaN())
    {
        std::cerr << "SetImuPoseVelocity: NaN detected in Rwb, twb, or Vwb, ignoring bad IMU pose update" << std::endl;
        return;
    }
    Eigen::Matrix3f Rwb_norm = IMU::NormalizeRotation(Rwb);
    if (Rwb_norm.hasNaN())
    {
        std::cerr << "SetImuPoseVelocity: SVD normalization produced NaN, ignoring bad IMU pose update" << std::endl;
        return;
    }
