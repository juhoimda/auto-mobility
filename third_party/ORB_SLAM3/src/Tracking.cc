...
bool Tracking::PredictStateIMU()
{
    ...
    if(mbMapUpdated && mpLastKeyFrame)
    {
        ...
        if (!Rwb2.hasNaN() && !twb2.hasNaN() && !Vwb2.hasNaN())
        {
            mCurrentFrame.SetImuPoseVelocity(Rwb2,twb2,Vwb2);
        }

        if (!mpLastKeyFrame->GetImuBias().bwx != !mpLastKeyFrame->GetImuBias().bwx) // check not NaN
        {
...
void Tracking::UpdateFrameIMU(const float s, const IMU::Bias &b, KeyFrame* pCurrentKeyFrame)
{
    if(!pCurrentKeyFrame)
    {
        cerr << "UpdateFrameIMU: pCurrentKeyFrame is null" << endl;
        return;
    }
    ...
    {
        cerr << "UpdateFrameIMU: mCurrentFrame IMU state incomplete" << endl;
    }
