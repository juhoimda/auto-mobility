...
Eigen::Matrix3f NormalizeRotation(const Eigen::Matrix3f &R){
    ...
    Eigen::Matrix3f R_norm = svd.matrixU() * svd.matrixV().transpose();
    if (R_norm.hasNaN())
        return Eigen::Matrix3f::Identity();
    return R_norm;
}
...
{
    if (dt != dt || dt <= 0.0f || acceleration.hasNaN() || angVel.hasNaN())
    {
        std::cerr << "IntegrateNewMeasurement: skipping bad IMU sample (dt=" << dt << ")" << std::endl;
        return;
    }
...
Eigen::Matrix3f Preintegrated::GetDeltaRotation(const Bias &b_)
{
    ...
    if (dbg.hasNaN() || dR.hasNaN())
    {
        std::cerr << "GetDeltaRotation: bias difference or dR is NaN" << std::endl;
...
{
    std::unique_lock<std::mutex> lock(mMutex);
    if (db.hasNaN() || dR.hasNaN())
    {
        std::cerr << "GetUpdatedDeltaRotation: state is NaN" << std::endl;
        return Eigen::Matrix3f::Identity();
    }
...
{
    std::unique_lock<std::mutex> lock(mMutex);
    if (db.hasNaN() || dV.hasNaN() || JVg.hasNaN() || JVa.hasNaN())
        return dV;
