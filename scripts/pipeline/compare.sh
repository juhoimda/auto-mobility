...
# 이전 global OMP=6 은 GPU 100% + CPU 572% sustained 를 만들어 강제 재부팅을 유발.
# 새 정책 (§7): cuVSLAM 3, RTAB 6, GPU-TSDF 4. 여기서는 보수적 기본값 4.
# 실제 fusion/slam worker는 isolated.py / cli.py에서 phase-specific으로 재정의한다.
...
# phase-specific overrides (caller may set before invoking compare.sh)
export CUVSLAM_OMP_THREADS="${CUVSLAM_OMP_THREADS:-3}"
export RTAB_OMP_THREADS="${RTAB_OMP_THREADS:-6}"
export FUSION_OMP_THREADS="${FUSION_OMP_THREADS:-4}"
_sanitize_path() {
    local var="$1" out=""
    local _oldifs="$IFS"
    IFS=:
...
if [ -z "$1" ]; then
    ...
    echo ""
    echo " 실행 모드 (상호 배타적, 기본값: --standard):"
    echo "   --preview   : 육안 검사용 3D OBJ 비교 모드 (RTAB & cuVSLAM 양쪽 모두 동일 조건 생성, 800 FUSE, 32 뷰)"
    echo "   --standard  : 기본 적응형 Coarse-to-Fine 탐색 및 최종 전달 (권장 Standard)"
    echo "   --quick     : 개발자 Sanity / 파이프라인 Smoke 체크 모드 (~100 frames, 초고속, 품질평가용 아님)"
