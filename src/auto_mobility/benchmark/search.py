"""
search.py — Multi-Axis Candidate Search Engine with Beam Search & Full Rebuild.

Implements Coarse-to-Fine Beam Search:
  - Phase A (SLAM Screening): Evaluates SLAM backends/conditions on fixed 10mm TSDF -> Selects Top-K SLAMs.
  - Phase B (Fusion Screening): Explores TSDF (20/10/8mm, conditional 5mm) and Direct Point Cloud on surviving SLAMs -> Selects Top-K Fusion Pipelines.
  - Phase C (Surface Screening): Explores Surface methods (TSDF direct, Poisson, BPA, Alpha, CGAL) on surviving Fusion candidates -> Selects Top-3 Finalists.
  - Phase D (Full Rebuild): Re-runs Full Reconstruction (stride=1, all train frames) and Full Fidelity evaluation on Top-3 Finalists -> Final Ranking & Best OBJ.
"""
...
class SearchEngine:
    """Executes multi-axis exploration stages with Beam Search, caching, decision trace, and Full Rebuild."""
    ...
    # ───────────────────────────────────────────────────────────
    # PHASE A: SLAM Backend & Condition Screening (Beam Width = Top 2~3)
    # ───────────────────────────────────────────────────────────
    def run_phase_a(
        self,
        ...
        """Runs Phase A SLAM screening and returns (all_results, top_slams: [(slam_name, traj_path)])."""
        print("\n==========================================================")
        print(f" 🚀 [PHASE A] SLAM Backend Screening (Beam Width: Top {self.beam_width_slam}, Mode: {self.mode.upper()})")
        print("==========================================================")
        ...
        for slam_k, traj_file in trajectories.items():
            ...
            spec = CandidateSpec(
                ...
                slam_backend=slam_k,
                fusion_method="tsdf",
                fusion_params={"voxel_size_m": 0.010, "depth_max_m": self.depth_max, "trunc_mult": self.trunc_mult},
                surface_method="tsdf_direct",
                frame_stride=self._screening_stride()
            )
            spec_hash = spec.compute_spec_hash()

            # Fast Pose Preflight Gate
            if pose_diagnostics and slam_k in pose_diagnostics and len(trajectories) > 1:
                ...
                if diag.get("status") == "FAIL" and cov < 0.30:
                    print(f"⚠️ [Phase A Preflight] {slam_k} 포즈 커버리지 극히 불량 ({cov*100:.1f}% < 30%) → TSDF 연산 생략")
                    self._log_decision("Phase A (SLAM)", cand_name, "PRUNED_POSE", f"Low pose coverage ({cov*100:.1f}%)")
            ...
            cached_summary = self.artifact_mgr.should_reuse_evaluation(cand_name, self.force, expected_spec_hash=spec_hash)
            if cached_summary:
                print(f"⏭️ Evaluation 재사용: {cand_name}")
                self.stats["cached_count"] += 1
                self._log_decision("Phase A (SLAM)", cand_name, "REUSED", "Reused evaluation from cache")
    ...
    # ───────────────────────────────────────────────────────────
    # PHASE B: Fusion Screening (TSDF vs DirectCloud across Beam SLAMs)
    # ───────────────────────────────────────────────────────────
    def run_phase_b(
        self,
        top_slams: List[Tuple[str, str]],
        ...
        """Runs Phase B Fusion exploration and returns (all_results, top_fusion_pipelines)."""
        print("\n==========================================================")
        ...
        for slam_name, traj_path in top_slams:
            ...
            for v in voxel_options:
                ...
                spec = CandidateSpec(
                    ...
                    fusion_method="tsdf",
                    fusion_params={"voxel_size_m": v, "depth_max_m": self.depth_max, "trunc_mult": self.trunc_mult},
                    surface_method="tsdf_direct",
                    frame_stride=self._screening_stride()
                )
                spec_hash = spec.compute_spec_hash()
                ...
                cand_10mm_alt = f"{slam_name}_voxel10mm"
                if v_mm == 10 and (cand_10mm_a in phase_a_by_cand or cand_10mm_alt in phase_a_by_cand):
                    existing = phase_a_by_cand.get(cand_10mm_a) or phase_a_by_cand.get(cand_10mm_alt)
            ...
            # 2. Direct Point Cloud Fusion Baseline (Bypassing TSDF)
            # Evaluate direct backprojection point cloud
            direct_v_mm = 10
            ...
            spec_direct = CandidateSpec(
                ...
                fusion_params={"voxel_size_m": 0.010, "depth_max_m": self.depth_max},
            ...
            if not is_artifact_valid(direct_pcd_out) or self.force:
                ...
                print(f"⏭️ Direct PCD 재사용: {direct_pcd_out.name}")
                self._log_decision("Phase B (Fusion)", direct_cand_name, "REUSED_PCD", "Reused existing direct point cloud")

            # Direct PCD surface representation (Poisson on Direct PCD for screening)
            direct_mesh_out = self.artifact_mgr.get_mesh_path(f"{slam_name}_direct", direct_v_mm, method="poisson")
            direct_eval_dir = self.artifact_mgr.get_candidate_eval_dir(f"{direct_cand_name}_poisson")
            cached_dir_eval = self.artifact_mgr.should_reuse_evaluation(f"{direct_cand_name}_poisson", self.force)
            ...
            elif is_artifact_valid(direct_pcd_out):
                ...
                if w_surf.is_success:
                    try:
                        dir_summary = evaluate_reconstruction(
                            ...
                            candidate_name=f"{direct_cand_name}_poisson",
                            split_json=str(self.split_file),
        ...
        # 3. Adaptive Fine Voxel (5mm) Check if improving
        # If fine 8mm or 10mm showed substantial gain and RAM is available, test 5mm
        if self.mode != "quick":
            valid_b_curr = [r for r in rank_candidate_summaries(fusion_eval_results) if r.get("hard_gate_pass", False)]
            if valid_b_curr:
                best_cand_b = valid_b_curr[0]
                best_slam_curr = best_cand_b["summary_data"].get("spec", {}).get("requested_params", {}).get("slam_backend") or top_slams[0][0]
                best_traj_curr = next((t[1] for t in top_slams if t[0] == best_slam_curr), top_slams[0][1])

                cand_5mm_name = f"{best_slam_curr}_tsdf5mm"
                mesh_5mm = self.artifact_mgr.get_mesh_path(best_slam_curr, 5)
                pcd_5mm = self.artifact_mgr.get_pcd_path(best_slam_curr, 5)
                eval_5mm_dir = self.artifact_mgr.get_candidate_eval_dir(cand_5mm_name)

                print(f"▶️ Adaptive Fine Voxel (5mm) Evaluation: {cand_5mm_name}")
                self.stats["total_candidates"] += 1
                w_res = run_tsdf_worker(
                    dataset_dir=str(self.dataset.dataset_dir),
                    traj_file=best_traj_curr,
                ...
                if w_res.is_success:
                    try:
                        sum_5mm = evaluate_reconstruction(
                            dataset_input=self.dataset,
                            trajectory_input=best_traj_curr,
    ...
    # ───────────────────────────────────────────────────────────
    # PHASE C: Surface Reconstruction Screening (across Top Fusion Pipelines)
    # ───────────────────────────────────────────────────────────
    def run_phase_c(
        self,
        ...
        """Runs Phase C Surface exploration and returns (all_results, top_finalists)."""
        print("\n==========================================================")
        ...
        for pipe_summary in top_fusion_pipelines:
            ...
            for sm in all_methods:
                # Direct point cloud does not have tsdf_direct
                ...
                # If TSDF direct, reuse existing TSDF mesh directly
    ...
    # ───────────────────────────────────────────────────────────
    # PHASE D: FULL REBUILD & Full-Fidelity Evaluation (Top 3 Finalists)
    # ───────────────────────────────────────────────────────────
    def run_full_rebuild(
        ...
        for rank_idx, f_summary in enumerate(finalists, 1):
            ...
            slam_name = spec_info.get("slam_backend") or cand_name.split("_")[0]
            traj_path = f_summary.get("trajectory_path") or trajectories.get(slam_name, "")
            fusion_method = f_summary.get("fusion_method", "tsdf")
            surface_method = f_summary.get("surface_method", "tsdf_direct")
            voxel_m = f_summary.get("voxel_size_m", 0.010)
            v_mm = int(round(voxel_m * 1000))

            print(f"\n🔨 [Full Rebuild #{rank_idx}] Candidate: `{rebuild_cand_name}`")
            print(f"   SLAM: {slam_name}, Fusion: {fusion_method} ({v_mm}mm), Surface: {surface_method}, Stride: 1 (FULL)")

            rebuild_pcd = self.artifact_mgr.get_pcd_path(f"{slam_name}_fullrebuild", v_mm)
            rebuild_mesh = self.artifact_mgr.get_mesh_path(f"{slam_name}_fullrebuild", v_mm, method=surface_method)
            rebuild_eval_dir = self.artifact_mgr.get_candidate_eval_dir(rebuild_cand_name)

            self.stats["total_candidates"] += 1
            t_rb_start = time.time()

            # 1. Full Rebuild Fusion (stride=1)
            ...
            # 2. Full-Fidelity Evaluation (high resolution depth maps, point-to-mesh, renders)
            try:
                ...
                self.stats["evaluated_count"] += 1
                self._log_decision("Phase D (Full Rebuild)", rebuild_cand_name, "EXECUTED", "Completed full rebuild & full-fidelity evaluation")
        ...
        winner = valid_final[0] if valid_final else (ranked_final[0] if ranked_final else None)

        if winner:
            self._log_decision("Final Ranking", winner["candidate_name"], "OVERALL_WINNER", f"Selected as overall winner after Full Rebuild (Quality: {winner.get('quality_score', 0):.1f}, Composite: {winner.get('composite_score', 0):.1f})")
            print("\n==========================================================")
    ...
    def compute_overall_rankings(
        ...
        run_full_eval_on_winner: bool = False
    ) -> Tuple[List[dict], Optional[dict]]:
        """Combine screening results from all phases into a joint ranking."""
        all_summaries = phase_a_results + phase_b_results + phase_c_results
